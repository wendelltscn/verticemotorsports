...
const LanguageSwitcher: React.FC<{ onSwitchSpam: () => void }> = ({ onSwitchSpam }) => {
    ...
    const tracker = React.useRef({ count: 0, lastTime: 0 });
...
const CockpitNav: React.FC<CockpitNavProps> = ({ onNavigate, activePage, isPlayerOpen, hasPlayerBeenLoaded, onTogglePlayer, isScrolled }) => {
    ...
    // Easter Egg States
    const [isGhostActive, setIsGhostActive] = React.useState(false);
    const [ghostMessage, setGhostMessage] = React.useState('');
    const [showFrequencyEgg, setShowFrequencyEgg] = React.useState(false);
    const holdTimer = React.useRef<ReturnType<typeof setTimeout> | null>(null);
    const frequencyAudioRef = React.useRef<HTMLAudioElement | null>(null);
    const [showLangEgg, setShowLangEgg] = React.useState(false);
    ...
    React.useEffect(() => {
        frequencyAudioRef.current = new Audio("data:audio/wav;base64,UklGRiYAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQIAAAD//wA/AD8APwA/AD8APwA/AD8A");
