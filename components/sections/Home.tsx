...
// FIX: Excluded 'Wallpapers' from the required keys, as it's not a hoverable item on the home grid and has no subtitle.
const pageSubtitles: Record<Exclude<Page, 'Home' | 'Movimento' | 'Wallpapers'>, string> = {
...
const Home: React.FC<HomeProps> = ({ onNavigate, intentPage, onBackgroundChange }) => {
    ...
    const [hoveredPage, setHoveredPage] = React.useState<Page | null>(null);
    const [isCenterHovered, setIsCenterHovered] = React.useState(false);
    const [hoveredGuideOption, setHoveredGuideOption] = React.useState<Page | null>(null);
    const [showFirstTimeHint, setShowFirstTimeHint] = React.useState(false);
    const [centerClickCount, setCenterClickCount] = React.useState(0);
    const [easterEggActive, setEasterEggActive] = React.useState(false);

    // Ignition Sequence Easter Egg
    const [hoverSequence, setHoverSequence] = React.useState<string[]>([]);
    const [ignitionActive, setIgnitionActive] = React.useState(false);
    const ignitionKeyAudioRef = React.useRef<HTMLAudioElement | null>(null);
    const ignitionStartAudioRef = React.useRef<HTMLAudioElement | null>(null);

    // Impatient Hover Easter Egg
    const [showImpatientEgg, setShowImpatientEgg] = React.useState(false);
    const hoverTimestamps = React.useRef<number[]>([]);
    const lastHoveredPage = React.useRef<Page | null>(null);
    const impatientEggTimer = React.useRef<ReturnType<typeof setTimeout> | null>(null);
