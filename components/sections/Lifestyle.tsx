...
const Lifestyle: React.FC = () => {
    ...
    // Easter Egg States
    const [thumbnailClicks, setThumbnailClicks] = useState(0);
    const [showQAEgg, setShowQAEgg] = useState(false);
    const thumbnailClickTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

    const [ctaClicks, setCtaClicks] = useState(0);
    const [ctaMessage, setCtaMessage] = useState('');
    const ctaClickTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
    const [detailsSecret, setDetailsSecret] = useState<number | null>(null);
