...
const Works: React.FC<WorksProps> = ({ onNavigate }) => {
    ...
    const [clickTracker, setClickTracker] = React.useState({ key: '', count: 0 });
    const [showIndecisiveEgg, setShowIndecisiveEgg] = React.useState(false);
    const clickTimer = React.useRef<ReturnType<typeof setTimeout> | null>(null);

    // Atelier Ignition Easter Egg
    const [ignitionSequence, setIgnitionSequence] = React.useState<string[]>([]);
    const [showIgnitionEgg, setShowIgnitionEgg] = React.useState(false);
    ...
    const [processSecret, setProcessSecret] = React.useState<number | null>(null);
